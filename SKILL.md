...
description: Use when an AI coding/research agent needs to run an end-to-end deep learning paper experiment involving public dataset discovery/download, preprocessing audit logs, resumable plans, anti-loop safeguards, per-project conda isolation, baseline reproduction, evidence-based mechanism innovation, fast candidate screening, M0-M3 ablations, experiment consistency checks, result figures, bilingual manuscript drafting, Chinese Word/DOCX formatting, claim-evidence tracing, and reproducible artifacts.
---
