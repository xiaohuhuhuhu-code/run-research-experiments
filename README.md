...
- 需要优先选择近 2 年可复现的主基线，并补充至少 2 个经典基线作为对照。
- 需要先分析改进方向，再设计创新点，避免随意修改模型。
- 需要设计 3 个有依据、有逻辑、逐步提升的有效创新版本：`M0` 为基线，`M1/M2/M3` 为经过筛选和验证的机制改进版本。
...
- 需要稳定依赖版本，可按需使用 WSL、代理或镜像，但要写入环境报告。
...
- 基线必须包含近 2 年主基线和经典对照；找不到可复现近 2 年基线时必须记录搜索失败和替代方案。
...
- `M1` 必须优于 `M0` 后才能固定，`M2` 必须优于 `M1` 后才能固定，`M3` 应为主指标最优；失败创新要记录并重新诊断、替换或细化。
...
- 实验结束后要打包原始实验依据，包括日志、指标、配置、曲线数据、表格数据、绘图输入和校验清单。
- 最后要生成 `final_work_record.md`，记录 AI 做过的工作、产物、失败尝试、阻塞和下一步。
